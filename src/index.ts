...
class SupportIonicAppFlowMonorepo extends Command {
  ...
  getRequirePackageFromDependencies(path: string): string[] {
    ...
    const { dependencies } = getObjectFromJsonFile(
      process.cwd() + `/${path}/package.json`
    );
    ...
    for (const packageName of appDependencies) {
       ...
       */
      try {
        const { dependencies, peerDependencies } = getObjectFromJsonFile(
          process.cwd() + `/${path}/node_modules/${packageName}/package.json`
        );
        ...
        throw new Error(
          '[WARNING] Can not find ' + packageName + '. Did you do npm install?'
        );
      }
      ...
      try {
        if (
          existsSync(
            process.cwd() + `/${path}/node_modules/${packageName}/plugin.xml`
          )
        ) {
          installPackage.push(packageName);
          this.log(`[${chalk.green('OK')}] script found ${packageName}`);
  ...
  copyRequireFiles(path: string): void {
    cli.action.start(
      '> ' +
        chalk.green(`cd ${path} && cp -r -f config.xml www ` + process.cwd())
    );
    execSync(`cd ${path} && cp -r -f config.xml www ` + process.cwd());
    cli.action.stop();
  }
}
